...
# Default IP range comes from the config's SNI.InputFile (v4 ranges).
# Add the v6 ranges alongside so both address families get scanned
# (requires the China box to have IPv6 connectivity — v6 probes will
# simply fail otherwise, no harm to v4 scanning).
exec "$BIN_DIR/gwsdb" scan \
	-scanner-config "$SCANNER_CONFIG" \
	-scanner-dir "$SCANNER_DIR" \
	-ip-range "$SCANNER_DIR/iprange/iprange_gws_6_a.txt"
