...
The home page no longer queries the DB or renders the IP list server-side on every hit — that was wasteful since `ip_pool` became a live view (window functions over all of `ip_checks`) rather than a maintained table (see `ip_pool` above). Instead `/static/home.js` fetches `/api/pool/version` on load, compares it against a cached copy in `localStorage` (`gwsdb_pool_v1`), and only fetches the full `/api/pool` payload — then renders rows client-side via the DOM API (never `innerHTML`, since PTR hostnames/country are derived from live untrusted DNS data) — when the version has moved. Both ingest and recheck write `ip_checks` rows, so `PoolVersion()` bumps on either, and a repeat visit in between is served entirely from `localStorage` with no request to `/api/pool` at all. JS-disabled visitors aren't left with an empty shell, though: `home.tmpl`'s `<head>` has a `<noscript><meta http-equiv="refresh" ...></noscript>` pointing at `/?nojs=1` (the HTML parser honors this regardless of script execution), and `handleHome` treats `nojs=1` the same as a crawler UA — full server-rendered table. CSP's `connect-src 'self'` was added to allow the JS-path fetches.

`handleHome` (`server.go`) special-cases known bots/archivers (`isCrawlerUA`, substring match on User-Agent — Googlebot, ia_archiver, archive.today, etc.): they get the old full server-rendered table instead of the JS shell, since search/social crawlers commonly don't run JS at all, and an archived snapshot's JS would otherwise replay against a live `/api/pool` at some later, unpredictable state (or a dead origin) instead of showing what was actually captured. `loadPool()` holds the shared query logic behind both this path and `/api/pool`. This path deliberately has no ETag/304: the production deployment sits behind Cloudflare with HTML Auto Minify on, which strips the origin's ETag in transit (its minified output no longer matches the hash), so no client downstream of Cloudflare ever sees one to send back — revisit if that changes.
