...
`handleHome` (`server.go`) special-cases known bots/archivers (`isCrawlerUA`, substring match on User-Agent — Googlebot, ia_archiver, archive.today, etc.): they get the old full server-rendered table instead of the JS shell, since search/social crawlers commonly don't run JS at all, and an archived snapshot's JS would otherwise replay against a live `/api/pool` at some later, unpredictable state (or a dead origin) instead of showing what was actually captured. That path still avoids doing the query on every crawl — it sets `ETag: "pool-<version>"` off the same `PoolVersion()` and answers `304` on a matching `If-None-Match`. `loadPool()` holds the shared query logic behind both this path and `/api/pool`.
